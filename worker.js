...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      ctx.waitUntil(
        ensureDatabaseSchema(env).catch((err) => {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      for (const event of messagingEvents) {
        ctx.waitUntil(
          processMessengerEvent(event, env).catch((err) => logError('processMessengerEvent', err))
        );
      }
  ...
  try {
    ...
    if (messageObj) {
      ...
      if (Array.isArray(messageObj.attachments)) {
        const imageAttachment = messageObj.attachments.find((att) => att.type === 'image');
...
async function processFacebookCommentEvent(value, env) {
  try {
    ...
    // نستخدم دالة تولد رد راقي (بسيطة أو عبر OpenAI لو متاح)
    ...
    for (const entry of entries) {
      ...
      for (const change of changes) {
        ...
        for (const message of messages) {
          ctx.waitUntil(
            processWhatsAppMessage(message, value, env).catch((err) => logError('processWhatsAppMessage', err))
          );
        }
  ...
  try {
    ...
    if (imageUrl && conversation && conversation.status === BOOKING_STATUS.AWAITING_RECEIPT) {
      const confirmationText =
        'شكرًا لحضرتكم، تم استلام صورة إيصال التحويل وسيتم مراجعتها من قبل فريق الاستقبال. سنعاود التواصل معكم لتأكيد الحجز.';
    ...
    // 7) استدعاء OpenAI مع Function Calling لاستخراج بيانات الحجز إن وُجدت
    ...
    try {
      await sendReply(
        'نعتذر عن هذا الإزعاج، نواجه ضغطًا مؤقتًا في الاستجابة. سيتم التواصل معكم من فريق الاستقبال في أقرب وقت ممكن.'
      );
    } catch (innerErr) {
...
async function handleBookingProgress({ env, channel, externalUserId, conversation, extractedBooking }) {
  try {
    ...
    const previousData = conversation?.collected_data ? safeJsonParse(conversation.collected_data, {}) : {};
...
function buildMainSystemPrompt(conversation) {
  ...
  const bookingStatusNote = conversation?.status
    ? `\n\nملاحظة داخلية (لا تُذكر للمريض حرفيًا): حالة الحجز الحالية للمحادثة هي "${conversation.status}".`
    : '';

  return `أنت "${CLINIC_INFO.assistantName}"، المساعد الرقمي لقسم الاستقبال وخدمة العملاء في "${CLINIC_INFO.clinicName}"، عيادة ${CLINIC_INFO.doctorName}.
${bookingStatusNote}

شخصيتك وأسلوبك:
- تتحدث بأسلوب راقٍ جدًا واحترافي.
- تبقي الردود مختصرة وواضحة ولطيفة.
- لا تشخّص أو تصف أدوية أو تحدد جرعات.
...
العنوان:
${CLINIC_INFO.address}
خريطة: ${CLINIC_INFO.mapUrl}

نظام الحجز:
عند طلب المريض حجز موعد، اجمع البيانات تدريجيًا (سؤال أو اثنين في كل خطوة):
1) الاسم الكامل
2) رقم الهاتف
3) سبب الزيارة
4) الوقت/التاريخ المفضل

بعد اكتمال البيانات الأربعة، اطلب تحويل قيمة الكشف (${CLINIC_INFO.prices.firstVisit} ${CLINIC_INFO.prices.currency}) بأسلوب لطيف، واطلب صورة الإيصال لاحقًا. لا تؤكد الحجز نهائيًا بنفسك؛ التأكيد النهائي من فريق الاستقبال.`;
}

function buildCommentSystemPrompt() {
  return `أنت "${CLINIC_INFO.assistantName}"، المساعد الرقمي لصفحة "${CLINIC_INFO.clinicName}" على فيسبوك.
مهمتك الآن: الرد على تعليق عام كتبه شخص أسفل منشور على الصفحة.
قواعد:
- لا تطلب بيانات شخصية في تعليق عام.
- لا تقدم تشخيصًا أو توجيهات طبية تفصيلية.
- أجب بأسلوب راقٍ ومختصر، وادعُ صاحب التعليق للتواصل عبر الرسائل الخاصة.`;
}
...
async function callOpenAIWithBookingTools({ env, systemPrompt, messages }) {
  ...
  try {
    ...
    // نعرّف الدالة (function) التي يمكن أن يستدعيها الموديل لاستخراج بيانات الحجز
    const functions = [
      {
        name: 'record_booking_info',
        description:
          'تسجيل أي بيانات حجز يقدمها المريض ضمن رسالته (الاسم، رقم الهاتف، سبب الزيارة، الموعد المفضل).',
        parameters: {
          type: 'object',
          properties: {
            fullName: { type: 'string', description: 'اسم المريض بالكامل، إن ذُكر' },
            phoneNumber: { type: 'string', description: 'رقم هاتف المريض، إن ذُكر' },
            visitReason: { type: 'string', description: 'سبب الزيارة أو الحالة، إن ذُكر' },
            preferredTime: { type: 'string', description: 'الموعد أو اليوم/الوقت المفضل، إن ذُكر' },
          },
          required: [],
        },
      },
    ...
    // دعم استجابة function_call (طريقة OpenAI الرسمية)
    const functionCall = messageObj?.function_call;
    if (functionCall?.arguments) {
      try {
        const args = JSON.parse(functionCall.arguments || '{}');
        bookingData = sanitizeExtractedBookingData(args);
      } catch (parseErr) {
        logError('callOpenAIWithBookingTools: function_call args parse', parseErr);
      }
    }

    // لبعض أصناف الاستجابات القديمة أو المخصصة
    if (!bookingData && Array.isArray(messageObj?.tool_calls)) {
      const bookingCall = messageObj.tool_calls.find((tc) => tc.function?.name === 'record_booking_info');
      if (bookingCall) {
        try {
    ...
    // إذا استدعى النموذج الأداة فقط بدون نص رد، نطلب ردًا نصيًا تكميليًا
...
function buildChatMessagesFromHistory(history, currentText, imageUrl) {
  ...
  if (imageUrl) {
    currentContent = currentContent ? `${currentContent}\n[أرفق المريض صورة]` : '[أرسل المريض صورة]';
  }
...
// Simple comment reply generator (fallback). If OPENAI_API_KEY present, try to call OpenAI for a nicer reply.
async function generateCommentReply(commentText, env) {
  // Minimal safe public reply inviting to DM
  const fallback = 'شكرًا لتواصلك! لخصوصية التفاصيل الطبية أو الحجز، يُرجى مراسلتنا عبر الرسائل الخاصة وسنساعدك بسرور.';
  try {
    if (!env.OPENAI_API_KEY) return fallback;

    const systemPrompt = buildCommentSystemPrompt();
    const messages = [{ role: 'system', content: systemPrompt }, { role: 'user', content: commentText }];

    const aiReply = await callOpenAI({ env, systemPrompt, messages, maxTokens: 120 });
    return aiReply || fallback;
  } catch (err) {
    logError('generateCommentReply', err);
    return fallback;
  }
}
...
async function sendWhatsAppMessage(toPhoneNumber, text, env) {
  try {
    if (!env.WHATSAPP_TOKEN || !env.WHATSAPP_PHONE_NUMBER_ID) {
      logError('sendWhatsAppMessage', new Error('WHATSAPP_TOKEN or WHATSAPP_PHONE_NUMBER_ID is not configured'));
      return;
    }
...
// ============================================================================
// 11) طبقة قاعدة البيانات D1 (Schema + CRUD)
// ============================================================================
...
async function ensureDatabaseSchema(env) {
  ...
  try {
    await env.DB.batch([
      ...
      env.DB.prepare(`CREATE INDEX IF NOT EXISTS idx_bookings_status ON ${TABLES.BOOKINGS} (status);`),
    ]);

    schemaEnsured = true;
  } catch (err) {
    logError('ensureDatabaseSchema', err);
    // لا نرمي الخطأ للخارج - النظام يجب أن يستمر
  }
}
...
async function getConversationStateSafe(env, channel, externalUserId) {
  try {
    ...
    const result = await env.DB
      .prepare(`SELECT * FROM ${TABLES.CONVERSATIONS} WHERE channel = ? AND external_user_id = ?`)
...
async function updateConversationStateSafe(env, channel, externalUserId, { status, bookingId, collectedData }) {
  try {
    ...
    // استخدام UPSERT (ON CONFLICT DO UPDATE)
    await env.DB
      .prepare(
        `INSERT INTO ${TABLES.CONVERSATIONS} (channel, external_user_id, status, booking_id, collected_data, updated_at)
         VALUES (?, ?, ?, ?, ?, datetime('now'))
    ...
    const result = await env.DB
      .prepare(
        `INSERT INTO ${TABLES.BOOKINGS}
         (channel, external_user_id, full_name, phone_number, visit_reason, preferred_time, status, created_at, updated_at)
    ...
    await env.DB
      .prepare(
        `UPDATE ${TABLES.BOOKINGS}
         SET full_name = ?, phone_number = ?, visit_reason = ?, preferred_time = ?, status = ?, updated_at = datetime('now')
         WHERE id = ?;`
      )
      .bind(fullName || null, phoneNumber || null, visitReason || null, preferredTime || null, status || BOOKING_STATUS.COLLECTING, bookingId)
      .run();
  } catch (err) {
  ...
  try {
    ...
    if (receiptImageUrl) {
      await env.DB
        .prepare(
          `UPDATE ${TABLES.BOOKINGS}
           SET status = ?, receipt_image_url = ?, updated_at = datetime('now')
           WHERE id = ?;`
        )
      ...
      await env.DB
        .prepare(`UPDATE ${TABLES.BOOKINGS} SET status = ?, updated_at = datetime('now') WHERE id = ?;`)
